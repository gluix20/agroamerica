...
with consolidado as (
      ...
      from agrodw.cub_matog_mv c
      union all      
      select dim_locacion, dim_actividad, dim_fecha, tipo_oper, 0 mat, mdo mdo
      from agrodw.cub_mdo_mv c
      union all
      select dim_locacion, dim_actividad, dim_fecha, tipo_oper, 0 mat, mdo mdo
      from agrodw.cub_mdo_h_mv c
)
...
where c.tipo_oper in ('COSTO')
and a.macro not in ('OTROS')
